...
\begin{enumerate}
    ...
    \end{jieda}
    \item  (2026$\cdot$金山区一模$\cdot$16)记曲线 $C : \dfrac{{\left| x\right| }^{n}}{a} + \dfrac{{\left| y\right| }^{n}}{b} = 1\left( {a > 0\text{且}b > 0,n > 0\text{且}n \in  \mathbf{R}}\right)$ 为 ${C}_{\left( a,b\right) }^{n}$ ,称其为 “超椭圆”. 命题\ding{172} : 直线 $l : {2x} + {9y} - {12} = 0$ 与超椭圆 ${C}_{\left( 3,2\right) }^{\dfrac{1}{2}}$ 有 3 个不同交点; 命题\ding{173} : 超椭圆 ${C}_{\left( 9,1\right) }^{8}$ 上的点到坐标原点距离的取值范围为 $\left\lbrack  {1,{2}^{\dfrac{5}{8}}}\right\rbrack$ . 则下列说法正确的是\dotfill{(\kh{B})}
